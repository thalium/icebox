...
std::unique_ptr<core::IRegisters> core::make_registers(FDP_SHM& shm)
{
    return std::make_unique<Reg>(shm);
}
...
opt<uint64_t> Reg::read_reg(reg_e reg)
{
    uint64_t value;
    const auto ok = FDP_ReadRegister(&shm_, 0, reg, &value);
    if(!ok)
...
bool Reg::write_reg(reg_e reg, uint64_t value)
{
    const auto ok = FDP_WriteRegister(&shm_, 0, reg, value);
    if(!ok)
...
opt<uint64_t> Reg::read_msr(msr_e reg)
{
    uint64_t value = 0;
    const auto ok = FDP_ReadMsr(&shm_, 0, reg, &value);
    if(!ok)
...
bool Reg::write_msr(msr_e reg, uint64_t value)
{
    const auto ok = FDP_WriteMsr(&shm_, 0, reg, value);
    if(!ok)
